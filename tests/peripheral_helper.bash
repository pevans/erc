...
_pc_run_impl() {
	local extra="$1"; shift
	local steps="${PC_STEPS:-100}"
	local keys_flag=""
	if [[ -n "${PC_KEYS:-}" ]]; then
		keys_flag="--keys $PC_KEYS"
	fi
	...
	run "$ERC_BIN" headless \
		...
		--start-at 0801 \
		--steps "$steps" \
		--watch-comp PCSlotCX,PCSlotC3,PCExpansion,PCIOSelect,PCIOStrobe,PCExpSlot \
		$extra \
		$keys_flag \
		"$TMP/test.dsk"
}

# pc_run LINE [LINE...] -- assemble and run headless, watching peripheral comp
# states. State log lands in $OUT/state.log.
pc_run() {
	_pc_run_impl "" "$@"
}

# pc_run_with_mem LINE [LINE...] -- like pc_run but also watches zero-page
# memory $00-$04 for storing readback values.
pc_run_with_mem() {
	_pc_run_impl "--watch-mem 00-04" "$@"
}
