package a2hires_test

import (
	"image/color"
	"testing"

	"github.com/pevans/erc/a2/a2hires"
	"github.com/pevans/erc/a2/a2mono"
	"github.com/pevans/erc/gfx"
	"github.com/stretchr/testify/assert"
)

// mockSegment implements the memory.Getter interface for testing.
type mockSegment struct {
	data [0x4000]uint8
}

func (m *mockSegment) Get(addr int) uint8 {
	if addr < 0x2000 || addr >= 0x4000 {
		return 0
	}

	return m.data[addr]
}

func (m *mockSegment) Get16(addr int) uint16 {
	lo := uint16(m.Get(addr))
	hi := uint16(m.Get(addr + 1))

	return lo | (hi << 8)
}

// setupScreen creates a fresh framebuffer for testing.
func setupScreen() {
	gfx.Screen = gfx.NewFrameBuffer(560, 384)
}

// rowAddresses copied from coords.go for test convenience.
var rowAddresses = []int{
	0x2000, 0x2400, 0x2800, 0x2C00, 0x3000, 0x3400, 0x3800, 0x3C00, // 0-7
	0x2080, 0x2480, 0x2880, 0x2C80, 0x3080, 0x3480, 0x3880, 0x3C80, // 8-15
	0x2100, 0x2500, 0x2900, 0x2D00, 0x3100, 0x3500, 0x3900, 0x3D00, // 16-23
	0x2180, 0x2580, 0x2980, 0x2D80, 0x3180, 0x3580, 0x3980, 0x3D80, // 24-31
	0x2200, 0x2600, 0x2A00, 0x2E00, 0x3200, 0x3600, 0x3A00, 0x3E00, // 32-39
	0x2280, 0x2680, 0x2A80, 0x2E80, 0x3280, 0x3680, 0x3A80, 0x3E80, // 40-47
	0x2300, 0x2700, 0x2B00, 0x2F00, 0x3300, 0x3700, 0x3B00, 0x3F00, // 48-55
	0x2380, 0x2780, 0x2B80, 0x2F80, 0x3380, 0x3780, 0x3B80, 0x3F80, // 56-63
	0x2028, 0x2428, 0x2828, 0x2C28, 0x3028, 0x3428, 0x3828, 0x3C28, // 64-71
	0x20A8, 0x24A8, 0x28A8, 0x2CA8, 0x30A8, 0x34A8, 0x38A8, 0x3CA8, // 72-79
	0x2128, 0x2528, 0x2928, 0x2D28, 0x3128, 0x3528, 0x3928, 0x3D28, // 80-87
	0x21A8, 0x25A8, 0x29A8, 0x2DA8, 0x31A8, 0x35A8, 0x39A8, 0x3DA8, // 88-95
	0x2228, 0x2628, 0x2A28, 0x2E28, 0x3228, 0x3628, 0x3A28, 0x3E28, // 96-103
	0x22A8, 0x26A8, 0x2AA8, 0x2EA8, 0x32A8, 0x36A8, 0x3AA8, 0x3EA8, // 104-111
	0x2328, 0x2728, 0x2B28, 0x2F28, 0x3328, 0x3728, 0x3B28, 0x3F28, // 112-119
	0x23A8, 0x27A8, 0x2BA8, 0x2FA8, 0x33A8, 0x37A8, 0x3BA8, 0x3FA8, // 120-127
	0x2050, 0x2450, 0x2850, 0x2C50, 0x3050, 0x3450, 0x3850, 0x3C50, // 128-135
	0x20D0, 0x24D0, 0x28D0, 0x2CD0, 0x30D0, 0x34D0, 0x38D0, 0x3CD0, // 136-143
	0x2150, 0x2550, 0x2950, 0x2D50, 0x3150, 0x3550, 0x3950, 0x3D50, // 144-151
	0x21D0, 0x25D0, 0x29D0, 0x2DD0, 0x31D0, 0x35D0, 0x39D0, 0x3DD0, // 152-159
	0x2250, 0x2650, 0x2A50, 0x2E50, 0x3250, 0x3650, 0x3A50, 0x3E50, // 160-167
	0x22D0, 0x26D0, 0x2AD0, 0x2ED0, 0x32D0, 0x36D0, 0x3AD0, 0x3ED0, // 168-175
	0x2350, 0x2750, 0x2B50, 0x2F50, 0x3350, 0x3750, 0x3B50, 0x3F50, // 176-183
	0x23D0, 0x27D0, 0x2BD0, 0x2FD0, 0x33D0, 0x37D0, 0x3BD0, 0x3FD0, // 184-191
}

// fillMemoryWithByte fills all hires memory with a repeating byte value.
func fillMemoryWithByte(m *mockSegment, byt uint8) {
	for row := range 192 {
		baseAddr := rowAddresses[row]
		for offset := range 40 {
			m.data[baseAddr+offset] = byt
		}
	}
}

// TestRenderWhite tests that consecutive on bits produce white.
func TestRenderWhite(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// All bits on produces white (consecutive on bits). Note: the very first
	// dot in each 7-bit group won't have a previous on bit, so there will be
	// some colored pixels at byte boundaries. We check that the majority of
	// pixels are white.
	fillMemoryWithByte(mem, 0x7F)

	a2hires.Render(mem, 0, 0, a2mono.None)

	white := color.RGBA{0xff, 0xff, 0xff, 0xff}
	whiteCount := 0
	totalCount := 0

	// Sample pixels across the screen
	for y := uint(0); y < 384; y += 20 {
		for x := uint(0); x < 560; x += 20 {
			pixel := gfx.Screen.GetPixel(x, y)
			totalCount++
			if pixel == white {
				whiteCount++
			}
		}
	}

	// Most pixels should be white (at least 80%)
	whitePercentage := float64(whiteCount) / float64(totalCount) * 100
	assert.Greater(t, whitePercentage, 80.0,
		"at least 80%% of pixels should be white, got %.1f%%", whitePercentage)
}

// TestRenderBlack tests that all bits off produces black.
func TestRenderBlack(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// All bits off (default zero memory)

	a2hires.Render(mem, 0, 0, a2mono.None)

	black := color.RGBA{0x00, 0x00, 0x00, 0xff}

	// Sample pixels across the screen
	for y := uint(0); y < 384; y += 50 {
		for x := uint(0); x < 560; x += 50 {
			pixel := gfx.Screen.GetPixel(x, y)
			assert.Equal(t, black, pixel,
				"pixel at (%d,%d) should be black", x, y)
		}
	}
}

// TestRenderPurple tests isolated on bits at even positions with purple/green
// palette.
func TestRenderPurple(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// Pattern 0x15 = 00010101 produces isolated on bits at positions 0, 2, 4
	// (even positions). High bit clear means purple/green palette.
	fillMemoryWithByte(mem, 0x15)

	a2hires.Render(mem, 0, 0, a2mono.None)

	purple := color.RGBA{0xd0, 0x43, 0xe5, 0xff}

	// Check that we see purple pixels
	purpleCount := 0
	for y := uint(0); y < 384; y += 20 {
		for x := uint(0); x < 560; x += 20 {
			pixel := gfx.Screen.GetPixel(x, y)
			if pixel == purple {
				purpleCount++
			}
		}
	}

	assert.Greater(t, purpleCount, 50, "should have many purple pixels")
}

// TestRenderGreen tests isolated on bits at odd positions with purple/green
// palette.
func TestRenderGreen(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// Pattern 0x2A = 00101010 produces isolated on bits at positions 1, 3, 5
	// (odd positions). High bit clear means purple/green palette.
	fillMemoryWithByte(mem, 0x2A)

	a2hires.Render(mem, 0, 0, a2mono.None)

	green := color.RGBA{0x2f, 0xbc, 0x1a, 0xff}

	// Check that we see green pixels
	greenCount := 0
	for y := uint(0); y < 384; y += 20 {
		for x := uint(0); x < 560; x += 20 {
			pixel := gfx.Screen.GetPixel(x, y)
			if pixel == green {
				greenCount++
			}
		}
	}

	assert.Greater(t, greenCount, 50, "should have many green pixels")
}

// TestRenderBlue tests isolated on bits at even positions with blue/orange
// palette.
func TestRenderBlue(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// Pattern 0x95 = 10010101 produces isolated on bits at positions 0, 2, 4
	// (even positions). High bit set means blue/orange palette.
	fillMemoryWithByte(mem, 0x95)

	a2hires.Render(mem, 0, 0, a2mono.None)

	blue := color.RGBA{0x2f, 0x95, 0xe5, 0xff}

	// Check that we see blue pixels
	blueCount := 0
	for y := uint(0); y < 384; y += 20 {
		for x := uint(0); x < 560; x += 20 {
			pixel := gfx.Screen.GetPixel(x, y)
			if pixel == blue {
				blueCount++
			}
		}
	}

	assert.Greater(t, blueCount, 50, "should have many blue pixels")
}

// TestRenderOrange tests isolated on bits at odd positions with blue/orange
// palette.
func TestRenderOrange(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// Pattern 0xAA = 10101010 produces isolated on bits at positions 1, 3, 5
	// (odd positions). High bit set means blue/orange palette.
	fillMemoryWithByte(mem, 0xAA)

	a2hires.Render(mem, 0, 0, a2mono.None)

	orange := color.RGBA{0xd0, 0x6a, 0x1a, 0xff}

	// Check that we see orange pixels
	orangeCount := 0
	for y := uint(0); y < 384; y += 20 {
		for x := uint(0); x < 560; x += 20 {
			pixel := gfx.Screen.GetPixel(x, y)
			if pixel == orange {
				orangeCount++
			}
		}
	}

	assert.Greater(t, orangeCount, 50, "should have many orange pixels")
}

// TestRenderMonochromeGreen tests green monochrome rendering.
func TestRenderMonochromeGreen(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// Create a checkerboard pattern
	for row := range 192 {
		baseAddr := rowAddresses[row]
		for offset := range 40 {
			if offset%2 == 0 {
				mem.data[baseAddr+offset] = 0x55
			} else {
				mem.data[baseAddr+offset] = 0x2A
			}
		}
	}

	a2hires.Render(mem, 0, 0, a2mono.GreenScreen)

	green := a2mono.Green
	black := a2mono.Black

	greenCount := 0
	blackCount := 0

	for y := uint(0); y < 384; y += 10 {
		for x := uint(0); x < 560; x += 10 {
			pixel := gfx.Screen.GetPixel(x, y)
			switch pixel {
			case green:
				greenCount++
			case black:
				blackCount++
			default:
				t.Errorf("unexpected color at (%d,%d): got %+v, want green or black",
					x, y, pixel)
			}
		}
	}

	assert.Greater(t, greenCount, 0, "should have some green pixels")
	assert.Greater(t, blackCount, 0, "should have some black pixels")
}

// TestRenderMonochromeAmber tests amber monochrome rendering.
func TestRenderMonochromeAmber(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// Create a checkerboard pattern
	for row := range 192 {
		baseAddr := rowAddresses[row]
		for offset := range 40 {
			if offset%2 == 0 {
				mem.data[baseAddr+offset] = 0x55
			} else {
				mem.data[baseAddr+offset] = 0x2A
			}
		}
	}

	a2hires.Render(mem, 0, 0, a2mono.AmberScreen)

	amber := a2mono.Amber
	black := a2mono.Black

	amberCount := 0
	blackCount := 0

	for y := uint(0); y < 384; y += 10 {
		for x := uint(0); x < 560; x += 10 {
			pixel := gfx.Screen.GetPixel(x, y)
			switch pixel {
			case amber:
				amberCount++
			case black:
				blackCount++
			default:
				t.Errorf("unexpected color at (%d,%d): got %+v, want amber or black",
					x, y, pixel)
			}
		}
	}

	assert.Greater(t, amberCount, 0, "should have some amber pixels")
	assert.Greater(t, blackCount, 0, "should have some black pixels")
}

// TestRenderMonochromeAllOn tests monochrome with all bits on.
func TestRenderMonochromeAllOn(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	fillMemoryWithByte(mem, 0x7F)

	a2hires.Render(mem, 0, 0, a2mono.GreenScreen)

	green := a2mono.Green

	// All pixels should be green
	for y := uint(0); y < 384; y += 20 {
		for x := uint(0); x < 560; x += 20 {
			pixel := gfx.Screen.GetPixel(x, y)
			assert.Equal(t, green, pixel,
				"all pixels should be green when all bits are on")
		}
	}
}

// TestRenderMonochromeAllOff tests monochrome with all bits off.
func TestRenderMonochromeAllOff(t *testing.T) {
	setupScreen()
	mem := &mockSegment{}

	// All memory is zero by default

	a2hires.Render(mem, 0, 0, a2mono.AmberScreen)

	black := a2mono.Black

	// All pixels should be black
	for y := uint(0); y < 384; y += 20 {
		for x := uint(0); x < 560; x += 20 {
			pixel := gfx.Screen.GetPixel(x, y)
			assert.Equal(t, black, pixel,
				"all pixels should be black when all bits are off")
		}
	}
}
