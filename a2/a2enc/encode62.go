...
const (
	// SixBlock is the length of a data block that is the "six" of six-and-two
	// encoding.
	SixBlock = 0x100
...
// different byte value that would be literally written to and read from the
// disk media. Apple II's RWTS subroutine would then translate them back into
// data that is useful to the software being run.
//
...
// An encoder is a struct which defines the pieces we need to encode logical
// data into a physical format.
type encoder struct {
...
// Write will write a set of bytes into the destination segment at the current
// offset.
...
// writeByte simply writes a single byte into the physical segment without
// having to deal with passing around a slice
func (e *encoder) writeByte(byt uint8) {
	e.physicalSegment.Set(e.physicalOffset, byt)
	e.physicalOffset++
}

// encodeTrack will write a physically encoded track into the destination
// segment based on a logically encoded source.
...
func (e *encoder) writeDataField(track, sect int) {
	...
	// Loop on the logical sector data block and build up the six-block and
	// two-block buffers
	for i := range 0x100 {
		...
		// And then we encode the "last" two bits by OR-ing them with other
		// two-bit segments we've seen.
		two[i%TwoBlock] |= rev << ((i / TwoBlock) * 2)
	}

	// As we write out the physical sector data block, we must XOR each byte
	// with each other byte. But the first byte is written unmodified.
	e.writeByte(encGCR62[two[0]])
	for i := uint(1); i < TwoBlock; i++ {
		e.writeByte(encGCR62[two[i]^two[i-1]])
	}

	// A similar strategy is employed while writing the six-block buffer, with
	// the note that we must XOR the final two-block byte with the initial
	// six-block one.
	...
	// We still need to write out the last byte of the six-block buffer, but
	// here, there's no need to XOR since there's no other byte.
...
// encode4n4 writes the given byte in 4-and-4 encoded form, which is used in
// sector headers.
