...
func (s *a2Suite) TestUseDefaults() {
	...
	s.False(s.comp.State.Bool(statemap.BankReadRAM))
	s.True(s.comp.State.Bool(statemap.BankWriteRAM))
	s.False(s.comp.State.Bool(statemap.BankDFBlockBank2))
	s.False(s.comp.State.Bool(statemap.BankSysBlockAux))
}
...
func (s *a2Suite) TestSwitchRead() {
	...
	rd := func(addr int) bool {
		...
		return s.comp.State.Bool(statemap.BankReadRAM)
	}
	...
	wr := func(addr int) bool {
		...
		return s.comp.State.Bool(statemap.BankWriteRAM)
	}

	df := func(addr int) bool {
		_ = bankSwitchRead(int(addr), s.comp.State)
		return s.comp.State.Bool(statemap.BankDFBlockBank2)
	}
	...
	s.Run("bit 7 is high", func() {
		...
		s.comp.State.SetBool(statemap.BankDFBlockBank2, true)
		s.Equal(hi7, bankSwitchRead(int(0xC011), s.comp.State))
		s.comp.State.SetBool(statemap.BankDFBlockBank2, false)
		s.Equal(lo7, bankSwitchRead(int(0xC011), s.comp.State))

		s.comp.State.SetBool(statemap.BankReadRAM, true)
		s.Equal(hi7, bankSwitchRead(int(0xC012), s.comp.State))
		s.comp.State.SetBool(statemap.BankReadRAM, false)
		s.Equal(lo7, bankSwitchRead(int(0xC012), s.comp.State))

		s.comp.State.SetBool(statemap.BankSysBlockAux, true)
		s.Equal(hi7, bankSwitchRead(int(0xC016), s.comp.State))
		s.comp.State.SetBool(statemap.BankSysBlockAux, false)
...
func (s *a2Suite) TestSwitchWrite() {
	...
	s.Run("switching main to aux", func() {
		...
		s.True(s.comp.State.Bool(statemap.BankSysBlockAux))
...
func (s *a2Suite) TestBankDFRead() {
	...
	testForBankAux := func(useAux bool) {
		s.comp.State.SetBool(statemap.BankSysBlockAux, useAux)
...
func (s *a2Suite) TestBankDFWrite() {
	...
	testForBankAux := func(useAux bool) {
		...
		s.Run("writes respect the value of the write mode", func() {
			s.comp.State.SetBool(statemap.BankReadRAM, true)
			s.comp.State.SetBool(statemap.BankWriteRAM, true)
			s.comp.State.SetBool(statemap.BankDFBlockBank2, false)
			s.comp.Set(dfaddr, val1)
			s.Equal(val1, s.comp.Get(dfaddr))

			s.comp.State.SetBool(statemap.BankWriteRAM, false)
...
func (s *a2Suite) TestBankZPRead() {
	...
	for _, c := range cases {
		...
		s.comp.State.SetBool(statemap.BankSysBlockAux, c.useAux)
