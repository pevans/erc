...
func writePreamble(fp *os.File) {
	preamble := `* This file is automatically generated by Erc when it executed the contents of
* some software. It contains instructions written in an MOS 6502 assembly
* notation. It is designed to be readable so that you can understand what's
* happening while the software is running. Here's a typical line:
*
...
* "0804", it is encoded as "04" followed by "08".
*
* "MAIN" is a label. Space is always made for labels in the output, but labels
* aren't representable in machine code. Since Apple software typically jumps
* to address 0801, we can assert this is the starting point of the software,
* and thus we label this as MAIN. Feel free to add your own labels when
* reading an instruction log!
*
